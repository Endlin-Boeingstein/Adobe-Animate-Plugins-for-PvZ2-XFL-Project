...
// This plugin was made by Endlin Boeingstein 2024/8/14
...
for(i in itemArray){
    ...
    if(itemArray[i].itemType=="movie clip"||itemArray[i].itemType=="graphic"){
        ...
        for(var ly=0;ly< symbollayers.length;ly++){
            ...
            an.trace("layer name:"+an.getDocumentDOM().getTimeline().getLayerProperty("name"));
            //检测是否为引导层
            if(an.getDocumentDOM().getTimeline().getLayerProperty("layerType")=="guide"){
                // 删除引导层
                an.getDocumentDOM().getTimeline().deleteLayer();
                //回落
                ly--;
                an.trace("GuideLayer is Deleted");
            }
        }
        // 返回到主场景
